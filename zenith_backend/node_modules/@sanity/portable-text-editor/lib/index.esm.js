...
function isEqualToEmptyEditor(children, types) {
  return children === void 0 || children && Array.isArray(children) && children.length === 0 || children && Array.isArray(children) && children.length === 1 && Element$1.isElement(children[0]) && children[0]._type === types.block.name && "style" in children[0] && children[0].style === types.styles[0].value && Array.isArray(children[0].children) && children[0].children.length === 1 && Text.isText(children[0].children[0]) && children[0].children[0]._type === "span" && children[0].children[0].text === "";
}
          ...
          if (range) {
            ...
            const nodes = Editor.nodes(editor, {
              ...
              match: node => {
                ...
                return node._type === types.span.name || !editor.isTextBlock(node) && Element$1.isElement(node);
              }
            });
    ...
    if (editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3) {
      ...
      if (prevSel && onSamePath && editor.isTextBlock(block)) {
        ...
        if (needToAdjust) {
          ...
          const isMergeUnset = (previousPatch2 == null ? void 0 : previousPatch2.type) === "set" && previousPatch2.path[3] === "text" && typeof previousPatch2.value === "string" && editor.isTextSpan(textChild) && isEqual(previousPatch2.value.slice(-textChild.text.length), block.children[childIndex].text);
          if (isMergeUnset) {
...
function createWithSchemaTypes(types) {
  return function withSchemaTypes(editor) {
    ...
    editor.isVoid = element => {
      return types.block.name !== element._type && (types.blockObjects.map(obj => obj.name).includes(element._type) || types.inlineObjects.map(obj => obj.name).includes(element._type));
    };
...
class PortableTextEditor extends React.Component {
  constructor(props) {
    ...
    if (props.incomingPatches$) {
      this.incomingPatches$ = props.incomingPatches$.pipe(tap(_ref16 => {
        let {
          patches
        } = _ref16;
...
function bufferUntil(emitWhen) {
  return source => defer(() => {
    let buffer = [];
    return source.pipe(tap(v => buffer.push(v)),
